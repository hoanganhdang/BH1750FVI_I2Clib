...
BH1750FVI::BH1750FVI(){  
  ...
  byte buf [2];
  if (I2c.read (address_value, sizeof buf, buf) == 0)
  {
    Intensity_value = buf [0] << 8 | buf [1];
  }
  else
  {
	Intensity_value = 0;
    Serial.println ("BH1750 not responding");
  }
  ...
  void BH1750FVI::I2CWriteTo(uint8_t DataToSend){
    I2c.write(address_value, DataToSend);
  }
